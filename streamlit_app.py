...
def load_data():
    ...
    df_retornos = pd.read_excel('retornos ativos.xlsx')
...
def plot_carteira(data, apolice):
    ...
    fig = px.bar(results_df, x='Período', y='Retorno', title=f'Retorno para a Apólice {apolice}',
    ...
    fig.update_layout(
        ...
        title_x=0.5,
        margin=dict(l=20, r=20, t=50, b=20),  # Ajuste das margens para centralizar
        width=600, height=400  # Ajuste do tamanho do gráfico
    )
    return fig
...
def plot_retornos_ativos(data_ativos, periodo, apolice):
    ...
    fig = px.bar(retornos_agrupados, x='Nome do Ativo', y='Retornos', title=f'Retorno {periodo.capitalize()} dos Ativos para a Apólice {apolice}',
...
def plot_maiores_menores_retornos(data_ativos, periodo, top_n=5):
    ...
    fig_maiores = px.bar(maiores, x='Nome do Ativo', y='Retornos', title=f'Maiores Retornos {periodo.capitalize()}',
                         labels={'Retornos': 'Retorno (%)', 'Nome do Ativo': 'Ativo'},
    ...
    fig_menores = px.bar(menores, x='Nome do Ativo', y='Retornos', title=f'Menores Retornos {periodo.capitalize()}',
                         labels={'Retornos': 'Retorno (%)', 'Nome do Ativo': 'Ativo'},
...
def export_pdf(images, output_path, title, resumo):
    ...
    for image_path in images:
        pdf.add_page()
        pdf.image(image_path, x=20, y=30, w=170, h=110)  # Ajustar a largura e a altura da imagem para centralizar
